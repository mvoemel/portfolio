...
export function Spotlight() {
  ...
  const searchableItems = useMemo<SpotlightResult[]>(() => {
    const appResults: SpotlightResult[] = dockApps.flatMap(({ type, name, icon }) => {
      if (!type) return []
    ...
    const fileResults: SpotlightResult[] = getAllFiles(fileSystemRoot).map((item) => {
          ...
          <ul className="py-2 max-h-[60vh] overflow-y-auto scrollbar-hide">
            {filteredItems.map((item, index) => {
              ...
              return (
                ...
                >
                  {item.icon ? (
                    <img src={item.icon} alt={item.name} className="size-7 object-contain" />
                  ) : (
                    IconComponent && <IconComponent className="size-7 opacity-80" />
                  )}

                  <span className="text-base truncate font-normal leading-none">{item.name}</span>
