...
function Terminal() {
  ...
  const handleCommand = (cmdString: string) => {
    ...
    const newLog: TerminalLine[] = [{ type: 'input', content: cmdString, path: currentFolder.name }]
    ...
    switch (cmd) {
        ...
        newLog.push({
          ...
          content: (
            <div className="space-y-1 text-gray-300">
              <p>
                <span className="text-orange-400">cat &lt;file&gt;</span> - See the content of
                &lt;file&gt;
              </p>
              <p>
                <span className="text-orange-400">open &lt;file&gt;</span> - Open the &lt;file&gt;
                with the available app
              </p>
              <p>
                <span className="text-orange-400">cd &lt;dir&gt;</span> - Move into &lt;dir&gt;, "cd
                .." for parent, "cd ~" for root
              </p>
              <p>
                <span className="text-orange-400">ls</span> - See files and directories in the
                current directory
              </p>
              <p>
                <span className="text-orange-400">clear</span> - Clear the screen
              </p>
              <p>
                <span className="text-orange-400">exit</span> - Close this window
              </p>
              <p>
                <span className="text-orange-400">uname</span> - Show build info
              </p>
              <p>
                <span className="text-orange-400">rm -rf /</span> - Don't do this
              </p>
            </div>
          ),
        })
          ...
          content: (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 max-w-lg">
              {currentFolder.children.map((child) => (
                <span
                  key={child.id}
                  className={child.kind === 'folder' ? 'text-blue-400 font-bold' : 'text-gray-300'}
                >
                  {child.name}
                </span>
              ))}
      ...
      case 'cd':
        if (!arg || arg === '~' || arg === '/') {
          ...
          const target = currentFolder.children.find(
            (c) => c.name.toLowerCase() === arg.toLowerCase() && c.kind === 'folder',
          )
          if (target) {
      ...
      case 'cat': {
        const fileToRead = currentFolder.children.find((c) => c.name === arg && c.kind === 'file')
        if (fileToRead && fileToRead.kind === 'file') {
          if (fileToRead.meta.type === 'text' || fileToRead.meta.type === 'code') {
            newLog.push({
              type: 'output',
              content: (
                <pre className="whitespace-pre-wrap font-mono">{fileToRead.meta.content}</pre>
              ),
            })
          } else {
            newLog.push({
